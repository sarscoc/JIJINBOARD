...
function emitIntegratedProfile(){if(integrationEmbedded&&parent!==window)parent.postMessage({type:"jijinboard-profile",profile:{id:state.profile.id,plName:state.profile.plName,plIcon:state.profile.plIcon||"",personas:state.profile.personas.map(persona=>({id:persona.id||"",name:persona.name,type:persona.type,icon:persona.icon||""}))}},location.origin)}
...
async function createRoom() {
  ...
  try {
    ...
    if(integrationEmbedded&&integrationBoardId&&parent!==window)parent.postMessage({type:"jijinboard-room-created",roomId:result.id,adminToken:result.adminToken,title:state.parsed.title},location.origin);
