"use strict";
(()=>{
  if(typeof state==='undefined'||typeof renderComments!=='function'||typeof jumpToComment!=='function')return;
  ...
  function model(){
    ...
    return{items,children,messageTabs,order,roots};
  }
  ...
  renderComments=function(){
    ...
    limit=Math.max(Math.min(limit||batchSize,Math.max(items.length,batchSize)),Math.min(batchSize,items.length||batchSize));
    ...
    list.scrollTop=Math.max(0,Math.min(scrollTop,list.scrollHeight-list.clientHeight));
    queueFill();
  };
  ...
  jumpToComment=function(id){
    if(!document.getElementById(`comment-${CSS.escape(id)}`)){
      ...
      if(index>=0&&index>=limit){limit=Math.min(items.length,index+Math.max(4,Math.ceil(batchSize/3)));renderComments()}
    }
