...
(async()=>{
  ...
  // board-integration's point API resolves global fetch at call time. Add the
  // current template id to every point PATCH without changing the existing UI.
  ...
  function scheduleRemote(id,record,delay=350){
    const key=String(id||"");if(!key||!record)return;
    clearTimeout(saveTimers.get(key));
    saveTimers.set(key,setTimeout(()=>{saveTimers.delete(key);putRemote(key,record)},delay));
  }
  ...
  async function hydrate(){
    ...
    hydrating=true;
    try{
      const result=await api(endpoint("")),remote=Array.isArray(result?.templates)?result.templates:[],local=await window.tplAllRecords();
      if(remote.length){
        const states=window.templateStates()||{};
        applying=true;
        try{
          for(const item of remote){const rec=item?.record;if(!rec?.id)continue;await rawSet(rec.id,rec);states[rec.id]=item?.templateState&&typeof item.templateState==="object"?item.templateState:{}}
  ...
  if(typeof rawSaveTemplateState==="function"){
    window.saveCurrentTemplateState=function(...args){
      ...
      if(id&&hydrated)window.tplGetRecord?.(id).then(rec=>rec&&scheduleRemote(id,rec,350)).catch(()=>{});
      return result;
    };
  }
