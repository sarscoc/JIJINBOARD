...
async function storeTab(env,roomId,logId,tabId,tabName,messages){
  const chunks=[];for(let i=0;i<messages.length;i+=CHUNK_SIZE)chunks.push(messages.slice(i,i+CHUNK_SIZE));if(!chunks.length)chunks.push([]);
...
export async function createStreamRoom(request,env){
  ...
  const listed=Array.isArray(body.tabs)?body.tabs.map(String).filter(Boolean):[];
  const grouped=new Map(),sourceByMessage=new Map();
  body.messages.forEach((message,index)=>{sourceByMessage.set(message,index);const tab=String(message?.tab||listed[0]||'メイン');if(!grouped.has(tab))grouped.set(tab,[]);grouped.get(tab).push({...message,sourceIndex:Number.isFinite(Number(message?.sourceIndex))?Number(message.sourceIndex):index})});
  ...
  try{
    const html=typeof body.originalHtml==='string'?body.originalHtml:'';let htmlKey=null;if(html){htmlKey=originalKey(parentRoomId,id);await env.LOGS.put(htmlKey,html,{httpMetadata:{contentType:'text/html; charset=utf-8'},customMetadata:{roomId:parentRoomId,logId:id}})}
    ...
    for(let order=0;order<tabs.length;order++){
      const tabName=tabs[order],tabId=randomToken(16);await env.DB.prepare('INSERT INTO log_tab(tab_id,log_id,tab_name,tab_sort_order) VALUES(?,?,?,?)').bind(tabId,id,tabName,order).run();await storeTab(env,parentRoomId,id,tabId,tabName,grouped.get(tabName)||[]);
    }
