...
function annotationFromBody(id,logId,body,who,imageKey=''){
  ...
  return {id,room_id:logId,message_id:clean(body?.messageId,140),end_message_id:clean(body?.endMessageId||body?.messageId,140),parent_id:clean(body?.parentId,120),start_offset:Number(body?.startOffset)||0,end_offset:Number(body?.endOffset)||0,quote:clean(body?.quote,2000),color:clean(body?.color||'yellow',40),author_id:who.plId,author_name:clean(body?.authorName,80),persona_name:clean(body?.personaName,80),persona_type:clean(body?.personaType||'PL',8),persona_icon:who.personaIcon||'',body:clean(body?.body,4000).trim(),created_at:now,updated_at:now,has_image:imageKey?1:0,like_count:0,liked_by_me:false};
}
...
export async function handleLogCommentMutation(request,env,logId,cid='',action='',executionContext=null){
  ...
  if(request.method==='POST'&&!cid){
    ...
    notify(ctx,roomId,'comment:create',{logId,annotation,version});return json({annotation,version},201);
  }
  ...
  if(request.method==='POST'&&cid&&action==='like'){
    ...
    notify(ctx,roomId,'comment:like',{logId,id:cid,likeCount,actorId:authorId,liked,version});return json({liked,likeCount,version});
  }
  ...
  if(request.method==='PATCH'&&cid&&cid!=='color'){
    ...
    await env.DB.batch([env.DB.prepare('UPDATE comment SET comment_body=?,comment_image_key=?,updated_at=CURRENT_TIMESTAMP WHERE comment_id=?').bind(text,imageKey||null,cid),env.DB.prepare('UPDATE log_comment_range SET marker_color=? WHERE comment_id=?').bind(color,cid)]);
