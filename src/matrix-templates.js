...
export async function handleMatrixTemplates(request,env,roomId,logId,templateId='',imageOnly=false){
  ...
  if(request.method==='GET'&&!templateId){
    ...
    return json({templates:rows.map(row=>{const definition=parseDefinition(row.template_definition);return{id:row.template_id,name:row.template_name,dataUrl:row.template_image_key?imageUrl(roomId,row.template_id):'',imageName:String(definition.imageName||''),createdAt:Number(definition.createdAt)||Date.parse(row.created_at)||Date.now(),updatedAt:Number(definition.updatedAt)||Date.parse(row.updated_at)||Date.now()}})});
  }

  if(request.method==='PUT'&&templateId){
    const body=await safeBody(request),record=body?.record&&typeof body.record==='object'?body.record:{},authorId=String(body?.authorId||'').slice(0,120),authorName=String(body?.authorName||'').trim().slice(0,80);
    if(!authorId||!authorName)return json({error:'先に発言者を登録してください'},400);
    await ensurePlIdentity(env.DB,authorId,authorName);
    const old=await env.DB.prepare('SELECT template_image_key,created_at FROM matrix_template WHERE room_id=? AND template_id=?').bind(roomId,templateId).first();
    ...
    const definition=JSON.stringify({imageName:String(record.imageName||'').slice(0,180),createdAt:Number(record.createdAt)||Date.now(),updatedAt:Number(record.updatedAt)||Date.now()});
    ...
    return json({ok:true,record:{id:templateId,name,dataUrl:key?imageUrl(roomId,templateId):'',imageName:String(record.imageName||''),createdAt:Number(record.createdAt)||Date.now(),updatedAt:Number(record.updatedAt)||Date.now()}});
  }
