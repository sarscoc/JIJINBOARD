...
async function isV2(db){
  ...
  const matrixPk=new Map((matrixPoint.results||[]).map(item=>[item.name,Number(item.pk)||0]));
  if(matrixPk.get('room_id')!==1||matrixPk.get('template_id')!==2||matrixPk.get('point_id')!==3)return false;
