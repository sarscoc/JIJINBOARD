...
async function isV2(db){
  ...
  const matrixTemplateColumns=new Set((matrixTemplate.results||[]).map(item=>item.name));
  if(!matrixTemplateColumns.has('template_image_key'))return false;
