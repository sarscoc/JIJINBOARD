...
export class RoomHub extends DurableObject{
  ...
  async fetch(request){
    ...
    if(url.pathname==='/notify'&&request.method==='POST'){try{const message=await request.json();this.broadcast({type:'refresh',action:clean(message?.action,40),data:cleanData(message?.data)},clean(message?.excludeClientId,100));return json({ok:true})}catch{return json({error:'Invalid message'},400)}}
