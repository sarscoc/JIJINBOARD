...
export class RoomHub extends DurableObject{
  ...
  async fetch(request){
    ...
    if(url.pathname==='/notify'&&request.method==='POST'){try{const message=await request.json();const action=clean(message?.action,40);if(action==='matrix-point')return json({ok:true,ignored:true});const data=cleanData(message?.data),type=action.startsWith('comment:')?'comment-change':'refresh';this.broadcast({type,action,data},clean(message?.excludeClientId,100));return json({ok:true})}catch{return json({error:'Invalid message'},400)}}
